...
(async()=>{const browser=await chromium.launch({headless:true,...(process.env.CHROME_PATH?{executablePath:process.env.CHROME_PATH}:{})});try{
 ...
 assert.equal(await page.locator('#main-view .home-reading-cards').count(),1);
 assert.equal(await page.locator('#main-view').evaluate(el=>el.lastElementChild.classList.contains('home-reading-cards')),true);
 ...
 await page.locator('.profile-hero--compact').waitFor();await page.locator('.profile-hero .badge-preview').first().waitFor();
