...
(async()=>{const browser=await chromium.launch({headless:true,...(process.env.CHROME_PATH?{executablePath:process.env.CHROME_PATH}:{})});try{
 ...
 await page.evaluate(()=>{
  ...
  BT.store.mergeRemoteUsers([friend]);BT.community.searchReaders=async()=>[friend];BT.community.getReaderProfile=async()=>({bio:'',interests:['Romans','Histoire']});
  const cover=document.createElement('canvas');cover.width=120;cover.height=180;const ink=cover.getContext('2d');ink.fillStyle='#ba6a42';ink.fillRect(0,0,120,180);ink.fillStyle='#fff7e8';ink.font='18px Georgia';ink.fillText('Mémoires',14,52);ink.fillText('d’Hadrien',14,79);ink.fillRect(14,110,92,2);
  const book={id:'b1',title:'Mémoires d’Hadrien',authors:['Marguerite Yourcenar'],status:'en-cours',isbn:'9782070369218',mediaType:'print',coverUrl:cover.toDataURL('image/jpeg')};
 ...
 await page.locator('#app-dialog').getByText('bio…',{exact:true}).waitFor();
 const options=page.locator('#app-dialog .reader-profile-menu');await options.locator(':scope > summary').click();await options.getByText('Confidentialité',{exact:true}).waitFor();await options.getByText('Vous êtes amis · Gérer',{exact:true}).waitFor();await options.locator(':scope > summary').press('Escape');assert.equal(await options.getAttribute('open'),null);
 const actualCover=profile.locator('.reader-current-shelf img').first();await actualCover.waitFor();await actualCover.evaluate(img=>img.decode());assert.ok(await actualCover.evaluate(img=>img.naturalWidth>0&&img.src.startsWith('data:image/jpeg;')));
 ...
 await current.getByRole('button',{name:'Découvrir Mémoires d’Hadrien'}).click();const dialog=page.locator('.reader-dialog').last();await dialog.getByRole('button',{name:'Ajouter à ma bibliothèque',exact:true}).click();const add=page.locator('.reader-dialog').last();await add.getByRole('button',{name:'Ajouter à mon espace'}).click();
 ...
 await profile.locator('.surface-color-picker>summary').click();await profile.getByRole('button',{name:'Sauge',exact:true}).click();assert.match(await bookcase.getAttribute('class'),/bookcase--sage/);
 await profile.locator('.reader-library-tools .notebook-filters > summary').click();await profile.getByRole('button',{name:'Couvertures',exact:true}).click();await profile.locator('.reader-library-grid').waitFor();
 await page.setViewportSize({width:390,height:844});await profile.locator('.reader-library-tools .notebook-filters > summary').click();await profile.getByRole('button',{name:'Lus',exact:true}).click();await profile.getByText('Aucun livre dans ce rayon pour le moment.',{exact:true}).waitFor();
 await profile.getByRole('button',{name:'Carnets',exact:true}).click();await profile.getByText(/Le temps et la transmission/).waitFor();assert.equal(await profile.getByRole('button',{name:'Lire le carnet'}).count(),0);
 ...
 assert.deepEqual(errors,[]);console.log('PASS: custom cover loaded, empty bio, profile options, interactions, replies, retry, add book without private data, shelves, notebook, preferences, revocation, mobile and desktop');
