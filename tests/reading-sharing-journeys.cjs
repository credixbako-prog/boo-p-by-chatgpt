...
(async()=>{
 ...
 try{
  ...
  const fullThought='Une pensée de lecture. '.repeat(50);await page.locator('#session-trace-draft').click();await page.locator('.thought-fullscreen .thought-writing').fill(fullThought);await page.getByRole('button',{name:'Terminer',exact:true}).last().click();
