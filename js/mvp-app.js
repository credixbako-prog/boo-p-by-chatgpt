...
(() => {
  ...
  const ui = {
    ...
    selectedLibraryBookId: null, memoryDeckKeys: [], memoryDirections: new Map(), memoryDeckSignature: '', memorySessionTotal: 0, memorySessionComplete: false,
  ...
  function init() {
    ...
    document.addEventListener('boop:club-publication-changed',e=>{ui.clubSpaces.delete(e.detail);loadClubSpace(e.detail);});
  ...
  function render(shouldFocus = false) {
    ...
    const paint = () => {
      ...
      observeNotebookPreviews(); BT.readingExperience.mountQuoteDots(view);
      view.querySelectorAll('[data-saved-cards]').forEach(host=>BT.readingCards.mount(host,{creation:true,layout:host.classList.contains('home-reading-cards')?'carousel':'list',compact:host.classList.contains('home-reading-cards'),period:'month'}));
      view.querySelectorAll('[data-annual-reports]').forEach(host=>BT.annualReports.mount(host));
      ...
      requestAnimationFrame(() => {
        ...
        if(ui.route==='community'&&ui.params.get('publication')&&ui.publicationLink!==location.hash){ui.publicationLink=location.hash;BT.readerProfile.openPublication(ui.params.get('publication'));}
  ...
  function getMemoryDeck(items) {
    ...
    if (signature !== ui.memoryDeckSignature) {
      ...
      ui.memoryDeckKeys = BT.readingExperience.deck('words',items,{refresh:true,key:item=>item.memoryKey}).map(item=>item.memoryKey);
      ui.memoryDirections = new Map(ui.memoryDeckKeys.map((key,index) => [key, index % 2 ? 'definition' : 'word']));
  ...
  function renderMemoryQuiz(item, position, total) {
    if (!ui.memoryDirections.has(item.memoryKey)) ui.memoryDirections.set(item.memoryKey, ui.memoryDirections.size % 2 ? 'definition' : 'word');
    const reverse = ui.memoryDirections.get(item.memoryKey) === 'definition' && Boolean(item.detail?.trim());
    const question = reverse ? `Que signifie « ${item.answer} » ? Retrouvez sa définition.` : item.question;
    const label = `Afficher la réponse : ${question}`;
    return `<article class="memory-card-shell" data-memory-key="${attr(item.memoryKey)}" data-memory-direction="${reverse ? 'definition' : 'word'}">
      ...
      <button class="memory-flip-card" type="button" data-action="flip-memory" data-front-label="${attr(label)}" aria-pressed="false" aria-label="${attr(label)}">
        <span class="memory-flip-card__inner">
          <span class="memory-flip-card__face memory-flip-card__front" aria-hidden="false"><span class="status-chip">${reverse ? 'Définition à retrouver' : esc(item.kind)}</span><span class="memory-flip-card__question">${esc(question)}</span><span class="memory-flip-card__gesture"><span aria-hidden="true">↻</span> Toucher pour retourner</span></span>
          <span class="memory-flip-card__face memory-flip-card__back" aria-hidden="true"><span class="eyebrow">${reverse ? esc(item.answer) : 'Réponse'}</span><strong class="memory-flip-card__answer${reverse ? ' memory-flip-card__answer--definition' : ''}">${esc(reverse ? item.detail || 'Aucune définition enregistrée pour ce mot.' : item.answer)}</strong>${!reverse && item.detail ? `<span class="memory-flip-card__detail">${esc(item.detail)}</span>` : ''}<small>${esc(item.source)}</small></span>
        </span>
      </button>
  ...
  function getQuizCandidates() {
    ...
    const personal = store.getLexicon().filter(item => ['word','expression','citation'].includes(item.kind)).map(item => {
    ...
    const entries = [...personal, ...QUIZ_EXAMPLES, {id:'quiz-word-1',demo:true,kind:'word',word:'Sérendipité',definition:'Découverte heureuse faite par hasard.'},{id:'quiz-word-2',demo:true,kind:'word',word:'Palimpseste',definition:'Manuscrit réutilisé après effacement du premier texte.'},{id:'quiz-word-3',demo:true,kind:'word',word:'Sagacité',definition:'Capacité à comprendre avec finesse et lucidité.'}];
    ...
    const questions = [];
    words.forEach(item=>{if(item.definition)questions.push({id:item.id+':word',entryId:item.demo?'':item.id,kind:'Mot à retrouver',prompt:'Quel mot correspond à cette définition ?\n'+memoryDefinitionClue(item.definition,item.word),choices:quizChoices(item.word,words.map(w=>w.word),item.id),correct:item.word,explanation:item.definition,source:[item.bookTitle,item.author].filter(Boolean).join(' · ')||'Mon lexique',bookId:item.bookId});});
    ...
    citations.forEach(item => {
      ...
      const parts = splitCitation(item.word) || (String(item.word).trim().includes(' ')?{beginning:String(item.word).trim().split(/\s+/)[0],ending:String(item.word).trim().split(/\s+/).slice(1).join(' ')}:null);
  ...
  function renderLexiconQuiz() {
    ...
    const personalCount = store.getLexicon().filter(item => ['word','expression','citation'].includes(item.kind)).length;
    const settings = store.getSettings();
    ...
    if (!ui.quizStarted) return `<section class="section-block lexicon-quiz" aria-labelledby="lexicon-quiz-title"><div class="section-heading"><div><p class="eyebrow">Mots, expressions & citations</p><h2 id="lexicon-quiz-title">Quiz de lecture</h2><p class="small muted">Sens, contexte, auteur et livre d’origine : dix questions tirées au hasard dans vos souvenirs.</p></div>${colorPicker}</div><div class="card quiz-intro quiz-surface quiz-surface--${quizColor}"><span class="quiz-intro__mark" aria-hidden="true">?</span><div><strong>${personalCount ? `${personalCount} souvenir${personalCount > 1 ? 's' : ''} personnel${personalCount > 1 ? 's' : ''} prêt${personalCount > 1 ? 's' : ''}` : 'Quiz de découverte prêt'}</strong><p class="small muted">Les exemples BOO-P complètent vos mots, expressions et citations lorsque le lexique est encore court.</p></div><button class="button button--primary" type="button" data-action="start-quiz" ${candidates.length ? '' : 'disabled'}>Lancer 10 questions</button></div>${settings.lastLexiconQuiz?`<p class="quiz-last-result small muted">Dernier résultat : ${Number(settings.lastLexiconQuiz.score)||0}/${Number(settings.lastLexiconQuiz.total)||10} · ${formatDate(settings.lastLexiconQuiz.date)}</p>`:''}</section>`;
    if (ui.quizFinished || !questions.length) return `<section class="section-block lexicon-quiz" aria-labelledby="lexicon-quiz-title"><div class="section-heading"><div><span class="quiz-celebration" aria-hidden="true">${ui.quizScore/questions.length>=.7?'✦':'❧'}</span><p class="eyebrow">${ui.quizScore/questions.length>=.7?'Bravo, vos souvenirs prennent vie !':'Chaque essai fait grandir la mémoire'}</p><h2 id="lexicon-quiz-title">${ui.quizScore}/${questions.length || 10} bonnes réponses</h2><p class="small muted">Les réponses difficiles reviendront plus tôt dans vos prochaines révisions.</p></div><div class="section-heading__actions"><button class="button button--secondary button--small" type="button" data-action="restart-quiz">Recommencer</button>${colorPicker}</div></div></section>`;
    ...
    return `<section class="session-view">
      ...
      <aside class="card card-pad session-panel">
        <p class="eyebrow">Progression et note</p><div class="form-grid">
          ...
          <div class="field"><span>Trace en brouillon</span><button id="session-trace-draft" class="session-thought-preview" type="button" data-action="edit-session-thought" aria-label="Écrire une Trace en brouillon"><span class="thought-content">${session.traceDraft ? BT.thoughtEditor.html(session.traceDraft,session.traceFormatting) : 'Écrivez ou dictez une Trace…'}</span></button></div>
        </div>
      </aside>
    </section>`;
  }
  ...
  window.addEventListener('boop:local-post-changed',()=>{if(ui.route==='community')render();});
  function renderPost(post) {
    ...
    return `<article class="card activity-card" data-post="${attr(post.id)}">
      <header class="activity-head"><span class="avatar">${esc(post.initials || initials(post.authorName))}</span><div class="activity-meta"><strong>${esc(post.authorName)}</strong><span>${activityLabel(post)} · ${relativeDate(post.date)}</span></div>
        ${BT.publications.menuHTML(post)}
      </header>
      ${renderPostPhoto(post)}
      ${post.bookTitle ? `<p class="eyebrow">${esc(post.bookTitle)}</p>` : ''}${BT.publications.previewHTML(post.readingKind ? BT.sharing.publicationHTML(post.readingKind,post.readingContent,post.text) : `<p class="activity-text">${esc(post.text)}</p>`,post.id)}
  ...
  function renderTrail() {
    if(ui.params.get('section')==='cards')ui.trailMode='cards';
    ...
    const modes = `<div class="trail-view-modes"><div class="filter-chips" role="group" aria-label="Présentation du Sentier"><button type="button" data-action="trail-mode" data-mode="map" aria-pressed="${ui.trailMode === 'map'}">Carte mentale</button><button type="button" data-action="trail-mode" data-mode="cards" aria-pressed="${ui.trailMode === 'cards'}">Cartes et bilans</button></div></div>`;
    if (ui.trailMode === 'cards') return `${modes}<section class="section-block" data-saved-cards></section><section class="section-block" data-annual-reports></section>`;
  ...
  function renderNotebookEntries(notebook) {
    ...
    const entries = [...thoughts,...store.getLexicon()].filter(item => (notebook ? item.kind===(notebook==='citations'?'citation':'thought') : ['word','expression'].includes(item.kind)) && (!selectedBook || item.bookId === selectedBook) && (kind === 'all' || item.kind === kind) && normalize(`${item.word} ${item.definition} ${item.bookTitle}`).includes(normalize(ui.lexiconQuery))).sort((a,b) => new Date(b.updatedAt) - new Date(a.updatedAt));
    return `<section class="lexicon-view"><div class="library-search-row"><label class="search-field" for="lexicon-search"><span aria-hidden="true">⌕</span><input id="lexicon-search" data-input="lexicon-search" type="search" value="${attr(ui.lexiconQuery)}" placeholder="${notebook ? (notebook==='citations'?'Chercher une citation…':'Chercher une pensée…') : 'Chercher un mot ou une expression…'}"></label><button class="button button--primary" type="button" data-action="${notebook ? 'capture-memory' : 'add-lexicon'}" data-book-id="${attr(selectedBook)}">${notebook ? '+ Garder' : '+ Mot'}</button></div><details class="notebook-filters"><summary>Filtres${kind !== 'all' || selectedBook ? ' · actifs' : ''}</summary><div class="filter-chips" role="group" aria-label="Type de souvenir">${Object.entries(labels).map(([kind,label]) => `<button type="button" data-action="notebook-filter" data-kind="${kind}" aria-pressed="${(Object.hasOwn(labels,ui.lexiconKind) ? ui.lexiconKind : 'all') === kind}">${label}</button>`).join('')}</div><label class="field notebook-book-filter">Livre<select data-change="notebook-book"><option value="">Tous mes livres</option>${store.getBooks().map(book => `<option value="${attr(book.id)}" ${selectedBook === book.id ? 'selected' : ''}>${esc(book.title)}</option>`).join('')}</select></label></details><p class="small muted" role="status">${entries.length} ${notebook ? 'souvenir' : 'entrée'}${entries.length > 1 ? 's' : ''}</p>${entries.length ? `<div class="lexicon-grid${notebook ? '' : ' lexicon-list'}">${entries.map(item => renderNotebookCard(item,labels)).join('')}</div>` : `<div class="empty-state"><h2>${notebook ? 'Votre carnet attend un souvenir.' : 'Votre lexique attend un premier mot.'}</h2><p>${notebook ? 'Gardez une pensée ou une citation.' : 'Conservez un mot ou une expression à retrouver.'}</p></div>`}</section>`;
  }


  let notebookPreviewObserver;
  function observeNotebookPreviews(){notebookPreviewObserver?.disconnect();notebookPreviewObserver=new ResizeObserver(entries=>{for(const {target} of entries){const more=target.nextElementSibling;if(more?.classList.contains('notebook-more'))more.hidden=target.scrollHeight<=target.clientHeight+1;}});document.querySelectorAll('[data-notebook-preview]').forEach(node=>notebookPreviewObserver.observe(node));}
  function renderNotebookCard(item,labels) {
    const thought=item.kind==='thought',book=store.getBookById(item.bookId);
    if (['word','expression'].includes(item.kind)) return `<details class="card lexicon-card lexicon-entry" data-lexicon-id="${attr(item.id)}"><summary><span>${esc(item.word)}</span></summary><div class="lexicon-entry-details"><span class="lexicon-kind">${esc(labels[item.kind])}</span>${item.definition ? `<p class="lexicon-definition">${esc(item.definition)}</p>` : '<p class="muted">Aucune définition enregistrée.</p>'}${item.note ? `<p class="lexicon-note small muted">${esc(item.note)}</p>` : ''}<footer>${book ? `<a class="notebook-book-chip" href="#book?id=${encodeURIComponent(book.id)}">${esc(item.bookTitle || book.title)}</a>` : esc(item.bookTitle || 'Sans livre associé')}${item.author ? `<span>${esc(item.author)}</span>` : ''}<span>${item.page ? `${book?.mediaType === 'audio' ? 'min.' : 'p.'} ${esc(item.page)} · ` : ''}${formatDate(item.updatedAt)}</span></footer><div class="card-actions notebook-card-actions"><button class="text-link" type="button" data-action="edit-lexicon" data-id="${attr(item.id)}">Modifier</button><button class="text-link" type="button" data-action="delete-lexicon" data-id="${attr(item.id)}">Supprimer</button><button class="text-link" type="button" data-action="share-reading" data-kind="${attr(item.kind)}" data-id="${attr(item.id)}">Partager</button></div></div></details>`;
    const content=thought?BT.thoughtEditor.html(item.word,item.formatting):esc(item.word);
    if(item.kind==='citation')return `<article class="card lexicon-card notebook-citation">${BT.readingExperience.quotation(item)}<div class="card-actions notebook-card-actions"><button class="text-link" type="button" data-action="edit-lexicon" data-id="${attr(item.id)}">Modifier</button><button class="text-link" type="button" data-action="delete-lexicon" data-id="${attr(item.id)}">Supprimer</button><button class="text-link" type="button" data-action="share-reading" data-kind="citation" data-id="${attr(item.id)}">Partager</button></div></article>`;
    return `<article class="card lexicon-card"><span class="lexicon-kind">${labels[item.kind]}</span><div class="notebook-preview" data-notebook-preview>${thought&&item.subject?`<h3>${esc(item.subject)}</h3>`:''}<${thought?'div':'h3'} class="notebook-text thought-content">${content}</${thought?'div':'h3'}>${item.definition&&item.definition!==item.word?`<p>${esc(item.definition)}</p>`:''}${item.note?`<p class="small muted">${esc(item.note)}</p>`:''}</div><button class="text-link notebook-more" type="button" data-action="view-memory" data-kind="${attr(item.kind)}" data-id="${attr(item.id)}" hidden>Voir plus</button><footer>${book?`<a class="notebook-book-chip" href="#book?id=${encodeURIComponent(book.id)}">${esc(item.bookTitle||book.title)}</a>`:'Sans livre associé'}<span>${item.page?`${book?.mediaType==='audio'?'min.':'p.'} ${item.page} · `:''}${formatDate(item.updatedAt)}</span></footer><div class="card-actions notebook-card-actions"><button class="text-link" type="button" data-action="${thought?'edit-thought':'edit-lexicon'}" data-id="${attr(item.id)}">Modifier</button><button class="text-link" type="button" data-action="${thought?'delete-thought':'delete-lexicon'}" data-id="${attr(item.id)}">Supprimer</button><button class="text-link" type="button" data-action="share-reading" data-kind="${attr(item.kind)}" data-id="${attr(item.id)}">Partager</button></div></article>`;
  }
  function openMemory(id,kind) {
    const item=kind==='thought'?store.getTraces().find(t=>t.id===id):store.getLexicon().find(t=>t.id===id);if(!item)return;
    const book=store.getBookById(item.bookId);
    openDialog({title:kind==='thought'?'Ma pensée':kind==='citation'?'Ma citation':'Mon lexique',eyebrow:book?.title||'Mon carnet',body:`<div class="memory-full thought-content">${item.subject?`<h3>${esc(item.subject)}</h3>`:''}${kind==='thought'?BT.thoughtEditor.html(item.text,item.formatting):esc(item.word)}${item.definition&&item.definition!==item.word?`<p>${esc(item.definition)}</p>`:''}${item.note?`<p>${esc(item.note)}</p>`:''}</div>`});
  }
  function openSessionThought() {
    const session=store.getActiveSession();if(!session)return;
    const account=BT.auth?.getCurrentUser()?.id;
    const book=store.getBookById(session.bookId);
    BT.thoughtEditor.open({bookTitle:book?.title,author:book?.authors?.join(', '),subject:session.traceSubject||'',onSubject:subject=>store.updateActiveSession({traceSubject:subject},session.id),text:session.traceDraft,formatting:session.traceFormatting,current:()=>BT.auth?.getCurrentUser()?.id===account&&store.getActiveSession()?.id===session.id,onChange:value=>store.updateActiveSession({traceDraft:value.text,traceFormatting:value.formatting},session.id),onClose:()=>{if(ui.route==='session')render();}});
  }
...
`;
  ...
  function renderBookDetail() {
    ...
    return `<a class="text-link" href="#path?tab=${wishlist ? 'library' : 'library'}">← Ma bibliothèque</a><section class="book-detail-head section-block">${cover(book,'large')}<div class="book-detail-copy"><div class="button-row"><span class="status-chip ${book.status === 'en-cours' ? 'status-chip--active' : ''}">${wishlist ? 'Wishlist' : STATUS_LABELS[book.status]}</span>${!wishlist ? `<span class="privacy-badge">${SITUATION_LABELS[book.situation]}</span>` : ''}<span class="media-chip">${MEDIA_LABELS[book.mediaType] || 'Livre'}</span>${book.historicalBeforeJoin ? '<span class="status-chip">Lu avant mon inscription</span>' : ''}</div><h1>${esc(book.title)}</h1><p class="muted">${esc(book.authors.join(', '))}</p><p>${esc(book.description || 'Aucun résumé pour cette édition.')}</p>${!wishlist ? `<div class="progress-track" aria-label="Progression ${pct(progress.value,progress.total)} %"><span style="--width:${pct(progress.value,progress.total)}%"></span></div><p class="small muted">${progress.label}${book.rating ? ` · ${ratingStars(book.rating)}` : ''}</p>` : '<p class="small muted">Envie de lecture conservée dans votre wishlist.</p>'}<div class="button-row">${wishlist ? `<button class="button button--primary" type="button" data-action="move-to-library" data-id="${attr(id)}">Ajouter à ma bibliothèque</button>` : `<button class="button button--primary" type="button" data-action="book-session" data-id="${attr(id)}">${openSession ? 'Reprendre la session' : 'Démarrer une session'}</button>`}<button class="button button--ghost" type="button" data-action="edit-book" data-id="${attr(id)}">Modifier</button><a class="button button--secondary" href="#path?tab=notebook&book=${encodeURIComponent(id)}">Ouvrir le carnet</a><button class="text-link" type="button" data-action="capture-memory" data-book-id="${attr(id)}">Garder quelque chose</button><button class="text-link" type="button" data-action="share-reading" data-kind="book" data-id="${attr(id)}">Partager ma lecture</button></div></div></section>
      <div class="grid-2">
        ...
        <section class="card card-pad"><div class="section-heading"><h2>Traces</h2><button class="text-link" type="button" data-action="quick-trace" data-book-id="${attr(id)}">Ajouter</button></div>${traces.length ? traces.map(trace => `<article class="history-item"><span class="history-item__icon">✦</span><div class="history-item__content"><div class="thought-content">${BT.thoughtEditor.html(trace.text,trace.formatting)}</div><span class="small muted">${trace.page ? `${audio ? 'min.' : 'p.'} ${trace.page} · ` : ''}${VISIBILITY_LABELS[trace.privacy] || 'Privé'}</span></div></article>`).join('') : '<p class="small muted">Aucune Trace pour ce livre.</p>'}</section>
  ...
  function renderProfile() {
    ...
    return `<section class="card profile-hero profile-hero--compact">
      <div class="profile-tools"><button type="button" class="icon-button" data-action="profile-customize" aria-label="Modifier mon profil" title="Modifier mon profil">${BT.sharing.editIcon}</button><a class="icon-button" href="#profile?section=settings" aria-label="Réglages du compte"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" aria-hidden="true"><path d="M4 7h16M4 17h16"/><circle cx="9" cy="7" r="3" fill="var(--boo-paper)"/><circle cx="15" cy="17" r="3" fill="var(--boo-paper)"/></svg></a></div>
      ...
      <div class="profile-summary-stats" aria-label="Mon parcours"><div><strong>${stats.booksRead}</strong><span>livres lus</span></div><div><strong>${Math.floor(stats.totalMinutes/60)} h ${String(stats.totalMinutes%60).padStart(2,'0')}</strong><span>de lecture</span></div><div><strong>${stats.streak}</strong><span>jours de série</span></div></div>
      ${renderLatestBadge(badges)}
      <div class="profile-actions"><button class="text-link" type="button" data-action="preview-reader">Voir mon profil <span aria-hidden="true">↗</span></button></div></section>
      ...
      <section class="section-block" id="profile-settings" tabindex="-1"><div class="section-heading"><h2>Compte et préférences</h2><button class="icon-button" type="button" data-action="toggle-theme" aria-label="Passer au thème ${settings.theme === 'dark' ? 'clair' : 'sombre'}" aria-pressed="${settings.theme === 'dark'}">${settings.theme === 'dark' ? '☀' : '☾'}</button></div><p class="sync-indicator" id="sync-indicator" role="status"></p><div class="settings-list">
        <div data-staff-entry hidden></div>
  ...
  function renderLatestBadge(badges) {
    const recent=badges.items.filter(b=>b.unlockedAt).sort((a,b)=>new Date(b.unlockedAt)-new Date(a.unlockedAt)||a.id.localeCompare(b.id)).slice(0,3);
    const acquired=badges.items.filter(b=>b.unlockedAt).length;
    return `<section class="section-block profile-badges" aria-labelledby="badges-title"><div class="section-heading"><h2 id="badges-title">Mes badges</h2><button class="text-link" type="button" data-action="open-badges">${acquired} / ${badges.items.length} · Tout voir</button></div>${recent.length ? `<div class="profile-badge-row">${recent.map((b,i)=>`<button class="badge-preview" type="button" data-action="open-badges" aria-label="${attr(b.name)}${i===0?' · Dernier badge':''}">${BT.badges.artwork(b)}<strong>${esc(b.name)}</strong>${i===0?'<small>Dernier badge</small>':''}</button>`).join('')}</div>` : '<p class="muted">Votre premier badge vous attend. Chaque lecture dessine votre parcours.</p>'}</section>`;
  }
  ...
  function openBadgesDialog() {
    const badges=store.getBadges(), acquired=badges.items.filter(b=>b.unlockedAt).length;
    openDialog({title:'Tous les badges',eyebrow:`${acquired} acquis · ${badges.items.length} étapes`,wide:true,body:`<p class="small muted">Le dernier badge obtenu accompagne automatiquement votre profil. Vos records détaillés restent personnels.</p><details class="profile-statistics"><summary>Mes records personnels</summary><div class="personal-records"><span><strong>${formatDuration(badges.records.longestSessionSeconds)}</strong><small>plus longue session</small></span><span><strong>${badges.records.longestStreakDays} jours</strong><small>plus longue série</small></span><span><strong>${badges.records.booksInBestMonth} livres</strong><small>meilleur mois</small></span></div></details>${Object.entries(BT.badges.families).map(([family,label])=>`<section class="section-block"><h3>${label}</h3><div class="badge-collection">${badges.items.filter(b=>b.family===family).map(b=>`<article class="badge-collection-item ${b.unlockedAt?'is-unlocked':'is-locked'}">${BT.badges.artwork(b)}<div><h4>${esc(b.name)}</h4><p>${esc(b.description)}</p>${b.unlockedAt?`<small>Obtenu le ${formatDate(b.unlockedAt)}</small>`:`<progress value="${Math.min(b.value,b.target)}" max="${b.target}" aria-label="Progression : ${attr(b.name)}"></progress><small>À acquérir · ${Math.min(b.target,Math.floor(b.value))} / ${b.target}</small>`}</div></article>`).join('')}</div></section>`).join('')}`});
  }
  ...
  function openFinishSessionDialog() {
    ...
    const citations = Array.isArray(session.citations) ? session.citations : [];
    render();
    openDialog({ title: 'Terminer ma lecture', eyebrow: 'Chronomètre en pause', body: `<form class="form-grid" data-form="finish-session"><p class="muted">Où vous êtes-vous arrêté dans <strong>${esc(book.title)}</strong> ?</p>${renderSessionPositionSlider(book, session.endPage, { id:'finish-session-page', name:'endPage' })}<label class="checkbox-row"><input type="checkbox" name="markRead" ${Number(session.endPage) >= Number(total) && total ? 'checked' : ''}> J’ai terminé ce livre</label><details class="optional-fields"><summary>Fiche de lecture · facultative</summary>${BT.readingExperience.sheetFields(book,session.readingSheetDraft||book.readingSheet)}</details>${session.traceDraft?'<p class="small muted">Votre pensée en brouillon sera conservée dans le Carnet.</p>':''}<textarea name="traceText" hidden>${esc(session.traceDraft||'')}</textarea><input type="hidden" name="formatting" value="${attr(JSON.stringify(session.traceFormatting||null))}"><div class="form-grid"><fieldset class="book-rating-field"><legend>Note du livre · facultative</legend>${ratingPicker(book.rating, 'finish-rating')}<input type="hidden" name="rating" id="finish-rating" value="${book.rating || ''}"><p class="small muted" id="finish-rating-description">${book.rating ? `${book.rating} étoiles sur 5.` : 'Choisissez une note de 1 à 5 étoiles.'}</p></fieldset><label class="checkbox-row"><input type="checkbox" name="share"> Préparer une publication après ma lecture</label><p class="small muted">Un aperçu modifiable vous permettra de choisir l’audience et de confirmer. Votre pensée reste privée tant que vous ne publiez pas.</p></div>${citations.length ? `<p class="small muted">${citations.length} citation${citations.length > 1 ? 's' : ''} conservée${citations.length > 1 ? 's' : ''} dans votre carnet.</p>` : ''}<button class="button button--primary" type="submit">Enregistrer ma lecture</button><button class="text-link" type="button" data-action="resume-from-finish">Revenir à ma lecture</button></form>` });
    const form=document.querySelector('form[data-form="finish-session"]');
    BT.readingExperience.bindSheet(form,book,value=>store.updateActiveSession({readingSheetDraft:value},session.id));
  }
  ...
  async function handleClick(event) {
    ...
    switch (action) {
      case 'edit-publication': { const post=store.getCommunity().posts.find(p=>p.id===id);if(post)BT.sharing.editPost(post);break; }
      case 'edit-session-thought': openSessionThought(); break;
      case 'edit-thought': openTraceDialog(null,id); break;
      case 'delete-thought': if(confirm('Supprimer cette pensée de votre carnet ?')){store.deleteTrace(id);render();} break;
      case 'view-memory': openMemory(id,trigger.dataset.kind); break;
      ...
      case 'refresh-citations': BT.readingExperience.quoteDeck(true); render(); break;
      case 'reading-sheet': BT.readingExperience.openSheet(id); break;
      ...
      case 'trail-mode': rememberTrailViewport(); ui.trailImmersive=false; ui.trailMode = trigger.dataset.mode; if(ui.params.has('section')){ui.params.delete('section');history.replaceState(null,'','#path?tab=trail');} render(); break;
      ...
      case 'open-isbn-camera': { const form=document.querySelector('[data-form=book]'),account=BT.auth.getCurrentUser()?.id;const file=await BT.photoFrame.camera();if(file&&form?.isConnected&&account===BT.auth.getCurrentUser()?.id){if(file.isbn){const lookup=document.getElementById('book-isbn-lookup');lookup.value=file.isbn;lookup.closest('form').requestSubmit();}else await readISBNPhoto(file,true);}break;}
      ...
      case 'save-monthly-card': { trigger.disabled=true;try{ui.savedMonthlyCard ||= await BT.readingCards.save(ui.monthlyReportCanvas,ui.monthlyReportData);trigger.textContent='Enregistrée dans le Sentier';const go=document.createElement('a');go.href='#path?tab=trail&section=cards';go.className='text-link';go.textContent='Voir mes cartes';go.onclick=()=>closeDialog();trigger.after(go);showToast(ui.savedMonthlyCard.synced?'Carte enregistrée en privé dans votre Sentier':'Carte enregistrée en privé sur cet appareil');}catch(error){trigger.disabled=false;showToast(error.message);} break; }
      ...
      case 'profile-customize': openDialog({title:'Personnaliser mon profil',body:'<div class="profile-customize-options"><button class="button button--secondary" type="button" data-action="edit-profile">Photo et identité</button><button class="button button--secondary" type="button" data-action="reader-preferences">Mon espace de lecteur</button></div>'}); break;
  ...
  function startLexiconQuiz() {
    const candidates = getQuizCandidates();
    const selected = [],shuffled=BT.readingExperience.shuffle(candidates),personal=shuffled.filter(q=>q.entryId);
    const pool=personal.length>=10?personal:[...personal,...shuffled.filter(q=>!q.entryId)];
    for(const kind of ['Mot à retrouver','Expression en situation','Suite de citation']){const question=pool.find(q=>q.kind===kind);if(question)selected.push(question);}
    // Prefer different saved entries before asking another angle on the same one.
    for(const question of pool)if(selected.length<10&&!selected.some(q=>q.id===question.id||(q.entryId&&q.entryId===question.entryId)))selected.push(question);
    for(const question of pool)if(selected.length<10&&!selected.some(q=>q.id===question.id))selected.push(question);
  ...
  function nextLexiconQuizQuestion() {
    ...
    if(!ui.quizAnswered)return;
    if (ui.quizIndex + 1 >= questions.length) {ui.quizFinished = true;store.saveSettings({lastLexiconQuiz:{score:ui.quizScore,total:questions.length,date:new Date().toISOString()}});}
    else ui.quizIndex += 1;
    ui.quizAnswered = false;
    ui.quizSelected = '';
    render();
  ...
  function submitTrace(form, data) {
    ...
    store.saveTrace({ id:data.get('id')||undefined, bookId, page: Number(data.get('page')) || 0, text: data.get('text'), subject:data.get('subject'), formatting:BT.thoughtEditor.normalize(data.get('text'),data.get('formatting')), privacy: 'private' });
  ...
  async function submitFinishSession(form, data) {
    ...
    if(form.elements.sheetType)BT.readingExperience.saveSheet(book.id,BT.readingExperience.readSheet(form));
    const saved = store.finishActiveSession({ endPage: clamp(data.get('endPage'), 0, total || 99999), rating: data.get('rating'), traceText, traceFormatting:BT.thoughtEditor.normalize(traceText,data.get('formatting')), markRead, share:false });
