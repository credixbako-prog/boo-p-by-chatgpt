...
(() => {
  ...
  function render(shouldFocus = false) {
    ...
    const paint = () => {
      ...
      observeNotebookPreviews();
      view.querySelectorAll('[data-saved-cards]').forEach(host=>BT.readingCards.mount(host,{creation:true,layout:host.classList.contains('home-reading-cards')?'carousel':'list',compact:host.classList.contains('home-reading-cards')}));
  ...
  function renderPost(post) {
    ...
    return `<article class="card activity-card" data-post="${attr(post.id)}">
      <header class="activity-head"><span class="avatar">${esc(post.initials || initials(post.authorName))}</span><div class="activity-meta"><strong>${esc(post.authorName)}</strong><span>${activityLabel(post)} · ${relativeDate(post.date)}</span></div>
        ${post.authorId !== 'me' ? `<details class="safety-menu"><summary aria-label="Modérer cette publication">•••</summary><div class="safety-menu__panel"><button type="button" data-action="report-post" data-id="${attr(post.id)}">Signaler la publication</button><button type="button" data-action="block-user" data-id="${attr(post.authorId)}">Bloquer ${esc(post.authorName)}</button></div></details>` : `<button class="icon-button publication-edit" type="button" data-action="edit-publication" data-id="${attr(post.id)}" aria-label="Modifier ma publication" title="Modifier ma publication">${BT.sharing.editIcon}</button>`}
      </header>
      ${renderPostPhoto(post)}
      ${post.bookTitle ? `<p class="eyebrow">${esc(post.bookTitle)}</p>` : ''}${post.readingKind ? BT.sharing.publicationHTML(post.readingKind,post.readingContent,post.text) : `<p class="activity-text">${esc(post.text)}</p>`}
  ...
  function openMemory(id,kind) {
    ...
    openDialog({title:kind==='thought'?'Ma pensée':kind==='citation'?'Ma citation':'Mon lexique',eyebrow:book?.title||'Mon carnet',body:`<div class="memory-full thought-content">${kind==='thought'?BT.thoughtEditor.html(item.text,item.formatting):esc(item.word)}${item.definition&&item.definition!==item.word?`<p>${esc(item.definition)}</p>`:''}${item.note?`<p>${esc(item.note)}</p>`:''}</div>`});
  }
  function openSessionThought() {
    const session=store.getActiveSession();if(!session)return;
    const account=BT.auth?.getCurrentUser()?.id;
    BT.thoughtEditor.open({text:session.traceDraft,formatting:session.traceFormatting,current:()=>BT.auth?.getCurrentUser()?.id===account&&store.getActiveSession()?.id===session.id,onChange:value=>store.updateActiveSession({traceDraft:value.text,traceFormatting:value.formatting},session.id),onClose:()=>{if(ui.route==='session')render();}});
  }
...
`;
  ...
  function openFinishSessionDialog() {
    ...
    BT.thoughtEditor.attach(form.elements.traceText,{formatting:session.traceFormatting,onChange:value=>store.updateActiveSession({traceDraft:value.text,traceFormatting:value.formatting},session.id)});
  }
  ...
  function submitTrace(form, data) {
    ...
    store.saveTrace({ id:data.get('id')||undefined, bookId, page: Number(data.get('page')) || 0, text: data.get('text'), formatting:BT.thoughtEditor.normalize(data.get('text'),data.get('formatting')), privacy: 'private' });
