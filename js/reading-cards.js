...
BT.readingCards=(()=>{
  ...
  async function list(owner,offset=0,published=false,period='all'){const account=who(),client=await ready(account);let q=client.from('reading_cards').select('id,user_id,month_key,title,caption,visibility,image_data,created_at').eq('user_id',owner).order('created_at',{ascending:false}).order('id').range(offset,offset+5);if(period!=='all')q=q.like('month_key',period==='year'?'____':'____-__');if(published)q=q.neq('visibility','private');const rows=result(await q)||[];if(account!==who())throw new Error('Le compte a changé.');return rows.map(card=>({...card,synced:true}));}
  ...
  async function save(canvas,data,{id=crypto.randomUUID()}={}){const account=scope();const card={id,user_id:account,month_key:data.monthKey,title:(data.period==='year'?'Mon année de lecture · ':'Mon mois de lecture · ')+data.label,caption:'',visibility:'private',image_data:canvas.toDataURL('image/jpeg',.9),created_at:new Date().toISOString(),synced:false};
  ...
  function image(card){const img=el('img');img.alt=card.title;img.loading='lazy';img.width=1080;img.height=1350;if(/^data:image\/jpeg;base64,[A-Za-z0-9+/=]+$/.test(card.image_data||''))img.src=card.image_data;return img;}
  ...
  function open(card,mine=false){const {d,body}=modal(card.title),account=scope();body.append(image(card));if(card.caption)body.append(el('p',card.caption,'saved-card-caption'));const actions=el('div',null,'button-row');actions.append(button('Télécharger',()=>download(card)));body.append(actions);
    body.prepend(BT.publications.menu(card,{table:'reading_cards',remote:!!card.synced||!!card.remote}));
    if(mine){if(!card.synced)body.append(el('p','Enregistrée sur cet appareil · à synchroniser','small muted'));}
  ...
  function mount(host,{owner=scope(),creation=false,published=false,layout='carousel',compact=false,period='all'}={}){
    ...
    async function load(next=false){const token=++generation;const status=el('p','Chargement des cartes…','small muted');status.setAttribute('role','status');if(!next&&!rows.length)host.replaceChildren(status);else if(next)host.append(status);
      try{
        let remote=[];if(who())remote=await list(owner,next?offset:0,published,period);
    ...
    function paint(){if(period!=='all')rows=rows.filter(c=>period==='year'?c.month_key?.length===4:c.month_key?.length===7);host.replaceChildren();const heading=el('div',null,'section-heading'),title=el('h3',period==='year'?'Mes bilans annuels':creation?'Mes cartes de lecture':'Cartes partagées'),controls=el('div',null,'saved-card-dots'),rail=el('div',null,layout==='list'?'saved-card-list':'saved-card-carousel saved-card-carousel--paged');rail.setAttribute('role','region');rail.setAttribute('aria-label',layout==='list'?'Mes cartes de lecture':'Cartes de lecture à faire défiler horizontalement');if(layout!=='list')rail.tabIndex=0;
      ...
      if(!rows.length&&!creation)rail.append(el('p',period==='year'?'Vos bilans annuels apparaîtront ici.':'Les cartes publiées apparaîtront ici.','small muted'));
  ...
  async function remove(card,expected=who()){const owner=card.user_id;if(card.synced||card.remote){const client=await ready(expected);let query=client.from('reading_cards').delete().eq('id',card.id);if(owner===expected)query=query.eq('user_id',expected);const rows=result(await query.select('id'));if(!rows?.length)throw new Error('Cette carte n’est plus accessible.');}if(expected!==who())throw new Error('Le compte a changé.');await local('readwrite',s=>s.delete(owner+':'+card.id));changed();}
  return {save,mount,open,list,clearLocal,exportLocal,editPublication,remove,cached};
})();
