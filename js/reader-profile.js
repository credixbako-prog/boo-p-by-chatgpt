      ...
      try{
        if(!cursor&&tab==='path'){
          ...
          if(reading.available&&reading.showCurrent){area.append(el('h3','En ce moment, je lis…'));const shelf=el('div',null,'reader-current-shelf');area.append(shelf);
            ...
            reading.books.forEach(b=>{const c=bookCard(b,userId,true),interactions=el('section');c.append(interactions);mountInteractions(interactions,{owner:userId,book:b.id},'Bonne lecture !');shelf.append(c);});if(reading.books.length){shelf.setAttribute('role','region');shelf.setAttribute('aria-label','Lectures en cours à faire défiler horizontalement');BT.readingExperience.mountCarousel(shelf,{label:'Livre'});}}
          if(featured.books.length){area.append(el('h3','Mes livres à découvrir'));const shelf=el('div',null,'reader-featured-shelf');featured.books.forEach(b=>shelf.append(bookCard(b,userId,true)));area.append(shelf);shelf.setAttribute('role','region');shelf.setAttribute('aria-label','Livres à découvrir à faire défiler horizontalement');BT.readingExperience.mountCarousel(shelf,{label:'Livre'});}
