...
BT.readerProfile=(()=>{
  ...
  function bookCard(book,userId,compact=false){const card=el('article',null,'reader-book-card'+(compact?' reader-book-card--compact':'')),info=el('div',null,'reader-book-info');
    const open=button(book.title,()=>openBook(userId,book.id),'reader-book-title');info.append(open,el('p',(book.authors||[]).join(', '),'small muted'),el('span',statuses[book.status]||'À lire','status-chip'));
    card.append(cover(book),info);const actions=el('div',null,'reader-card-actions');actions.append(button('Découvrir ce livre',()=>openBook(userId,book.id),'text-link'),button('＋ À ma bibliothèque',()=>addBook(book),'text-link'));card.append(actions);return card;
  }
  ...
  function mountInteractions(host,target,likeLabel='Encourager',expanded=false){
    ...
    host.classList.add('reader-interactions');const bar=el('div',null,'reader-card-actions'),notice=message(),details=el('details',null,'reader-traces'),summary=el('summary','Laisser une Trace'),list=el('div',null,'reader-trace-list');details.open=expanded;
    const like=button(likeLabel,()=>mutate(()=>api().encourage(target,state.mine,account)),'button button--ghost button--small');like.disabled=true;bar.append(like);details.append(summary,list);
    ...
    async function load(offset=0){const token=++request;try{const data=await api().thread(target,offset);if(!current()||token!==request)return;
      state={...data,rows:offset?[...state.rows,...data.rows]:data.rows};loaded=true;like.textContent=(state.mine?'✓ ':'')+likeLabel+' · '+state.count;like.setAttribute('aria-pressed',String(!!state.mine));like.disabled=false;send.disabled=false;audience.textContent=state.audience;summary.textContent='Laisser une Trace · '+state.total;notice.textContent='';renderRows();
  ...
  function mount(host,userId){const account=who();profiles.set(host,account);host.closest('dialog')?.addEventListener('close',()=>profiles.delete(host),{once:true});host.classList.add('reader-profile');const welcome=el('section',null,'reader-welcome'),stats=el('div',null,'reader-stats'),nav=el('nav',null,'reader-tabs filter-chips'),area=el('section',null,'reader-content');nav.setAttribute('aria-label','Le profil de lecture');host.append(welcome,stats,nav,area);
    let tab='path',filter='all',generation=0;const current=()=>host.isConnected&&account===who();
    ...
    async function load(cursor=''){const token=++generation;if(!cursor)area.replaceChildren();nav.querySelectorAll('button').forEach(b=>b.setAttribute('aria-pressed',String(b.dataset.tab===tab)));const pending=message();area.append(pending);
      try{
        ...
        if(tab==='library'){
          if(!cursor){const filters=el('div',null,'filter-chips');for(const [value,label] of [['all','Tous'],['en-cours','En cours'],['lu','Lus'],['a-lire','À lire']]){const b=button(label,()=>{filter=value;load();},'');b.setAttribute('aria-pressed',String(filter===value));filters.append(b);}area.append(filters);}
          ...
          const grid=el('div',null,'reader-library-grid');result.books.forEach(b=>grid.append(bookCard(b,userId)));area.append(grid);if(!result.books.length&&!cursor)area.append(message('Aucun livre dans ce rayon pour le moment.'));if(result.next)more(result.next);
  ...
  return {mount,openPublication,openBook,editPreferences,mountInteractions,cover};
})();
