    ...
    async function load(cursor=''){const token=++generation;if(!cursor)area.replaceChildren();nav.querySelectorAll('button').forEach(b=>b.setAttribute('aria-pressed',String(b.dataset.tab===tab)));const pending=message();area.append(pending);
      try{
        ...
        if(tab==='library'){
          ...
          if(!cursor){const options=el('details',null,'shelf-appearance');options.append(el('summary','Affichage de la bibliothèque'));const modes=el('div',null,'filter-chips');for(const [value,label] of [['shelf','Meuble'],['grid','Couvertures']]){const b=button(label,()=>{libraryView=value;load();},'');b.setAttribute('aria-pressed',String(libraryView===value));modes.append(b);}options.append(modes);if(libraryView==='shelf'){const colors=el('div',null,'bookcase-finish-picker');colors.setAttribute('role','group');colors.setAttribute('aria-label','Couleur du meuble');for(const [value,label] of finishes){const b=button('',()=>{finish=value;const box=area.querySelector('.reader-bookcase');if(box)box.className='bookcase reader-bookcase bookcase--'+finish;colors.querySelectorAll('button').forEach(c=>c.setAttribute('aria-pressed',String(c===b)));},'bookcase-finish-swatch bookcase-finish-swatch--'+value);b.setAttribute('aria-label',label);b.setAttribute('aria-pressed',String(finish===value));b.append(el('span'));colors.append(b);}options.append(colors);}area.append(options);}
