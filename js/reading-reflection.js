...
BT.reflection = (() => {
  ...
  function tab(name) {
    ...
    dialog.querySelector('.dialog-head .eyebrow').textContent=name==='notebook'?'Chaque lecture laisse une trace':'Un moment pour réfléchir';
  ...
  async function open(id,initial='chat') {
    ...
    $('[data-share-notebook]').onclick=()=>BT.sharing.open('notebook',book.id);
  ...
  async function print() {
    ...
    const d=win.document;d.title=`BOO-P — ${book.title}`;const style=d.createElement('style');style.textContent='@page{size:A4;margin:22mm}body{font:12pt/1.6 Georgia,serif;color:#142438;max-width:170mm;margin:20px auto}header{color:#547460;font:12pt Arial;display:flex;align-items:center;gap:20px;border-bottom:1px solid #6D8F7A;padding-bottom:16px}header img{width:140px;height:42px}header span{flex:1}h1{font-size:30pt;line-height:1.15}main{white-space:pre-wrap;overflow-wrap:anywhere}footer{margin-top:30px;font:10pt Arial;color:#547460}';d.head.append(style);
    const header=d.createElement('header'),logo=d.createElement('img'),label=d.createElement('span');logo.alt='BOO-P';logo.width=140;logo.height=42;label.textContent='Carnet de réflexion';header.append(logo,label);d.body.append(header);
    const loaded=new Promise(resolve=>{const timer=setTimeout(resolve,4000);logo.onload=logo.onerror=()=>{clearTimeout(timer);resolve();};});logo.src=new URL('assets/brand/boo-p-horizontal.svg',document.baseURI).href;
    for(const [tag,text] of [['h1',book.title],['p',book.authors.join(', ')],['main',exportText(!$('[data-editor]').hidden)],['footer','Carnet personnel · '+new Date().toLocaleDateString('fr-FR')]]){const el=d.createElement(tag);el.textContent=text;d.body.append(el);}await loaded;if(!win.closed)win.print();
  }
