...
BT.thoughtEditor = (() => {
  ...
  function open({title='Trace en brouillon',bookTitle='',author='',subject='',onSubject=()=>{},text='',formatting=null,onChange=()=>{},current=()=>true,onClose=()=>{}}={}) {
    ...
    const input=document.createElement('textarea');input.value=text;input.maxLength=1200;form.append(head);if(bookTitle){const meta=document.createElement('section');meta.className='thought-book-context';const name=document.createElement('strong'),by=document.createElement('p'),label=document.createElement('label'),field=document.createElement('input');name.textContent=bookTitle;by.textContent=author;label.className='field';label.textContent='Sujet :';field.value=subject;field.maxLength=240;field.placeholder='À compléter…';field.oninput=()=>{if(current())onSubject(field.value);else d.close();};label.append(field);meta.append(name,by,label);form.append(meta);}form.append(input);d.append(form);document.body.append(d);
