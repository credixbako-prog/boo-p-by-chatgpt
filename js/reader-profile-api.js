...
BT.readerProfileApi=(()=>{
  ...
  async function badges(id){await ready(id);return result(await api().from('reader_badges').select('badge_id,unlocked_at').eq('user_id',id)) || [];}
  async function latestBadge(id){await ready();return result(await api().rpc('get_reader_latest_badge',{target_user:id}));}
  async function saveBadges(items,expected){const id=await ready(expected);if(!items.length)return;return result(await api().from('reader_badges').upsert(items.map(b=>({user_id:id,badge_id:b.id,unlocked_at:b.unlockedAt})),{onConflict:'user_id,badge_id',ignoreDuplicates:true}));}
  ...
  async function publications(id,kind='all',offset=0,bookId=null){await ready();let q=api().from('community_posts').select('id,author_id,author_name,book_title,body,visibility,reading_kind,reading_source_id,activity_type,created_at,reading_content',{count:'exact'}).eq('author_id',id).in('visibility',['public','friends']).order('created_at',{ascending:false}).order('id').range(offset,offset+11);if(kind==='notebook')q=q.eq('reading_kind','notebook');else if(kind!=='all')q=q.eq('reading_kind',kind);if(bookId)q=q.eq('reading_source_id',bookId).in('reading_kind',['notebook','debut','fin']);const r=await q;return {rows:result(r)||[],count:r.count};}
  async function publication(id){await BT.auth.ready();return result(await api().from('community_posts').select('id,author_id,author_name,book_title,body,visibility,activity_type,reading_kind,reading_source_id,reading_content').eq('id',id).maybeSingle());}
  ...
  return {identity,preferences,savePreferences,badges,latestBadge,saveBadges,books,publications,publication,thread,trace,encourage,removeTrace};
})();
