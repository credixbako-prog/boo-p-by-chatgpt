...
BT.store = (() => {
  ...
  function makeBook(data = {}) {
    ...
    return {
      ...
      readingSheet: data.readingSheet && typeof data.readingSheet === 'object' ? clone(data.readingSheet) : null,
  ...
  function finishActiveSession(summary = {}, id = null) {
    ...
    if (summary.traceText) saveTrace({ bookId: session.bookId, sessionId: saved.id, text: summary.traceText, formatting:summary.traceFormatting ?? session.traceFormatting, subject:session.traceSubject, page: summary.endPage ?? session.endPage, privacy: summary.share ? 'public' : 'private' });
  ...
  function updateGroupPost(clubId,postId,text){const club=state.community.clubs.find(c=>c.id===clubId),post=club?.posts?.find(p=>p.id===postId);if(!post||post.authorId!=='me'||!String(text).trim())return null;post.text=String(text).trim().slice(0,1200);post.updatedAt=nowISO();commit({queue:'club.post'});return clone(post);}
  function removeGroupPost(clubId,postId){const club=state.community.clubs.find(c=>c.id===clubId),post=club?.posts?.find(p=>p.id===postId);if(!post||(post.authorId!=='me'&&!['owner','moderator'].includes(club.role)))return false;club.posts=club.posts.filter(p=>p.id!==postId);commit({queue:'club.post'});return true;}
