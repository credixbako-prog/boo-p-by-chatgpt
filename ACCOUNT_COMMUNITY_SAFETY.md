...
Le menu du profil d'un autre lecteur propose un signalement enregistré dans `user_reports`. L'auteur peut consulter ses propres signalements ; la personne signalée ne peut pas les lire. Un rôle de modération attribué **côté serveur** dans `app_metadata` peut traiter les signalements. Un simple champ `user_metadata` ne donne aucun droit de modération.

Les signalements de publications continuent d'utiliser `publication_reports`. Le responsable de l'application doit consulter ces deux files dans Supabase, examiner les éléments et les faire passer de `pending` à `reviewed` ou `dismissed` selon la décision. Les droits exacts de la file de publications restent ceux de sa migration existante. Aucun e-mail ou message de modération externe n'est envoyé automatiquement par cette livraison. Il reste nécessaire de désigner la personne responsable et une adresse d'assistance réellement consultée avant publication publique.
